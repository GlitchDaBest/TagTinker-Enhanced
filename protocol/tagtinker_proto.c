...
static const TagTinkerProfileEntry profile_table[] = {
    ...
    {TAGTINKER_TYPE_HD110_RED, 400, 300, TagTinkerTagKindDotMatrix, TagTinkerTagColorRed,
     "SmartTag HD110 Red", 0},
    /* F62 small SmartTag HD S red (type 1339, 152×152). */
    {TAGTINKER_TYPE_F62_HD_S_RED, 152, 152, TagTinkerTagKindDotMatrix, TagTinkerTagColorRed,
     "F62 SmartTag HD S Red", 0},
    /* SmartTag HD110 (type 1314): 400x300 mono graphic — striped IR when needed. */
    {TAGTINKER_TYPE_HD110, 400, 300, TagTinkerTagKindDotMatrix, TagTinkerTagColorMono,
     "SmartTag HD110", 0},
    ...
    /* SmartTAG Color M 2.2": type 1605 is the recovered tag. It accepts the
     * broadcast-image path with the 208x112 two-plane layout; keep 296x152 as
     * a separate 1695 experiment instead of breaking the known-working type. */
    {1503, 208, 112, TagTinkerTagKindDotMatrix, TagTinkerTagColorRed,    "Pricer Color M 2.2", 0},
...
const char* tagtinker_led_color_name(uint8_t color_index) {
    static const char* names[TAGTINKER_LED_COLOR_COUNT] = {
        "Red",
        "Green",
        "Blue",
        "Yellow",
    ...
    if(color_index >= TAGTINKER_LED_COLOR_COUNT) return names[0];
    return names[color_index];
}

uint8_t tagtinker_led_pricer_color_index(uint8_t color_index) {
    /* UI order: Red, Green, Blue, Yellow, Cyan, Magenta, White */
    static const uint8_t pricer[TAGTINKER_LED_COLOR_COUNT] = {
        4U, /* red */
        2U, /* green */
        0U, /* blue */
        6U, /* yellow */
    ...
    if(color_index >= TAGTINKER_LED_COLOR_COUNT) return pricer[0];
    return pricer[color_index];
}

uint8_t tagtinker_led_color_subcommand_classic(uint8_t color_index) {
    static const uint8_t subcmds[TAGTINKER_LED_COLOR_COUNT] = {
        0xC8, /* Red */
        0xC9, /* Green (PrecIR) */
        0xCA, /* Blue */
        0xCD, /* Yellow */
        0x49, /* Cyan (Pricehax alt) */
        0xCB, /* Magenta */
        0xCC, /* White */
    };
    if(color_index >= TAGTINKER_LED_COLOR_COUNT) return subcmds[0];
    return subcmds[color_index];
}

bool tagtinker_led_subcmd_use_classic_map(uint16_t type_code) {
    switch(type_code) {
    case TAGTINKER_TYPE_F62_HD_S_RED:
    case TAGTINKER_TYPE_HD110:
    case 1317U:
    case 1322U:
    case 1639U:
    case 1327U:
    ...
    case 1336U:
        return true;
    case TAGTINKER_TYPE_COLOR_M_22:
    case 1503U:
    case 1695U:
    case TAGTINKER_TYPE_COLOR_73:
        return false;
    default:
        return true;
    }
...
bool tagtinker_led_subcmd_use_classic_for_tx(const TagTinkerApp* app) {
    ...
    if(app->selected_target >= 0 && app->selected_target < (int8_t)app->target_count) {
        return tagtinker_led_subcmd_use_classic_map(
            app->targets[app->selected_target].profile.type_code);
    }
    /* Broadcast LED Blink / Colors: default classic for F62 1339 in the aisle. */
    return true;
}

uint8_t tagtinker_led_color_subcommand(uint8_t color_index) {
    /* Scan map (0xC1..0xC4 hits): Blue soft, Red strong, still green hue. */
    static const uint8_t subcmds[TAGTINKER_LED_COLOR_COUNT] = {
        0xC3, /* Red — strong green flash */
        0xC9, /* Green — strong (PrecIR) */
        0xC1, /* Blue — soft/normal green flash */
        0xCD, /* Yellow — strong */
        0xC2, /* Cyan — scan hit */
        0xC4, /* Magenta — scan hit */
        0xC0, /* White — try */
    };
    if(color_index >= TAGTINKER_LED_COLOR_COUNT) return subcmds[0];
    return subcmds[color_index];
...
void tagtinker_fill_led_blink_subcmd_map(
    ...
    uint8_t color_index,
    bool classic_map) {
    const uint8_t sub = classic_map ? tagtinker_led_color_subcommand_classic(color_index) :
                                      tagtinker_led_color_subcommand(color_index);
    uint16_t dur = duration;
    if(forever) {
        dur = 0xFFFFU;
    } else if(dur == 0U) {
        dur = classic_map ? TAGTINKER_LED_FLASH_DURATION_CLASSIC_SEC : TAGTINKER_LED_FLASH_DURATION_SEC;
    }
    ...
    payload[2] = 0x00;
    payload[3] = 0x00;
    payload[4] = (uint8_t)((dur >> 8) & 0xFF);
    payload[5] = (uint8_t)(dur & 0xFF);
}
...
void tagtinker_fill_led_blink_payload(
    ...
    if(forever) {
        ...
        dur = TAGTINKER_LED_FLASH_DURATION_SEC;
    }

    const uint8_t dur_hi = (uint8_t)((dur >> 8) & 0xFF);
    const uint8_t dur_lo = (uint8_t)(dur & 0xFF);

    const uint8_t p7 = tagtinker_led_pricer_color_index(color_index);

    if(enc == TagTinkerLedEncPricerIdx) {
        payload[0] = 0x06;
        payload[1] = 0x49;
        payload[2] = p7;
        payload[3] = 0x00;
    ...
    if(enc == TagTinkerLedEncPricerByte3) {
        payload[0] = 0x06;
        payload[1] = 0x49;
        payload[2] = 0x00;
        payload[3] = p7;
    ...
    if(enc == TagTinkerLedEncPricerMask) {
        payload[0] = 0x06;
        payload[1] = 0x49;
        payload[2] = (uint8_t)(1U << (p7 & 7U));
        payload[3] = 0x00;
        payload[4] = dur_hi;
        payload[5] = dur_lo;
        return;
    ...
    tagtinker_fill_led_blink_subcmd_map(payload, dur, forever, color_index, subcmd_classic_map);
}
...
void tagtinker_fill_page_select_payload(
    ...
    uint16_t duration) {
    payload[0] = 0x06;
    payload[1] = (uint8_t)(((page & 7U) << 3) | 0x01U | (forever ? 0x80U : 0x00U));
    payload[2] = 0x00;
    payload[3] = 0x00;
    payload[4] = (uint8_t)((duration >> 8) & 0xFF);
    payload[5] = (uint8_t)(duration & 0xFF);
}
...
size_t tagtinker_build_broadcast_led_frame(
    ...
    const uint8_t plid[4] = {0};
    uint8_t payload[6];
    tagtinker_fill_led_blink_payload(payload, duration, forever, color_index, enc, true);
    return tagtinker_make_addressed_frame(buf, plid, payload, sizeof(payload));
}
