 ...
 * Enhanced fork contributions by GlitchDaBest
...
bool tagtinker_target_supports_graphics(const TagTinkerTarget* target) {
    ...
    /* Segment tags are not full-frame bitmap. Unknown/missing profile must not expose image UX. */
    return target->profile.kind == TagTinkerTagKindDotMatrix;
}
...
void tagtinker_prepare_text_tx(TagTinkerApp* app, const uint8_t plid[4]) {
    ...
    app->image_tx_job.broadcast =
        plid && plid[0] == 0U && plid[1] == 0U && plid[2] == 0U && plid[3] == 0U;
...
bool tagtinker_try_broadcast_fav_bmp(TagTinkerApp* app, uint8_t slot) {
    if(!app || slot < 1U || slot > 3U) return false;

    Storage* storage = furi_record_open(RECORD_STORAGE);
    File* file = storage_file_alloc(storage);
    char path[TAGTINKER_IMAGE_PATH_LEN + 1];
    snprintf(path, sizeof(path), APP_DATA_PATH("dropped/fav%u.bmp"), (unsigned)slot);
    ...
    tagtinker_prepare_bmp_tx(
        app, broadcast_plid, path, 800U, 480U, app->img_page ? app->img_page : 0U);
    return true;
}

void tagtinker_start_broadcast_quick_text(TagTinkerApp* app, const char* text) {
    if(!app || !text) return;
    strncpy(app->text_input_buf, text, sizeof(app->text_input_buf) - 1U);
...
void tagtinker_settings_load(TagTinkerApp* app) {
    ...
    app->fast_large_mono = false;
    app->restore_page_after_image = false;
    app->led_color = 0U;
    app->led_encoding = (uint8_t)TagTinkerLedEncSubcmd;
    app->led_scan_cmd = 0U;
    ...
    if(storage_file_open(file, APP_DATA_PATH("settings.txt"), FSAM_READ, FSOM_OPEN_EXISTING)) {
        ...
        int warn, rep, sig, fast = 0, restore = 0, led_enc = (int)TagTinkerLedEncSubcmd;
        if(sscanf(buf, "%d|%d|%d|%d|%d|%d", &warn, &rep, &sig, &fast, &restore, &led_enc) == 6) {
            app->show_startup_warning = (warn != 0);
            app->data_frame_repeats = (uint16_t)rep;
            app->fast_large_mono = (fast != 0);
            app->restore_page_after_image = (restore != 0);
            if(led_enc >= 0 && led_enc <= (int)TagTinkerLedEncPricerMask) {
                app->led_encoding = (uint8_t)led_enc;
            }
        } else if(sscanf(buf, "%d|%d|%d|%d|%d", &warn, &rep, &sig, &fast, &restore) == 5) {
            app->show_startup_warning = (warn != 0);
            app->data_frame_repeats = (uint16_t)rep;
            app->fast_large_mono = (fast != 0);
            app->restore_page_after_image = (restore != 0);
        } else if(sscanf(buf, "%d|%d|%d|%d", &warn, &rep, &sig, &fast) == 4) {
            app->show_startup_warning = (warn != 0);
            app->data_frame_repeats = (uint16_t)rep;
            app->fast_large_mono = (fast != 0);
        } else if(sscanf(buf, "%d|%d|%d", &warn, &rep, &sig) == 3) {
            app->show_startup_warning = (warn != 0);
            app->data_frame_repeats = (uint16_t)rep;
...
bool tagtinker_settings_save(const TagTinkerApp* app) {
    ...
    if(storage_file_open(file, APP_DATA_PATH("settings.txt"), FSAM_WRITE, FSOM_CREATE_ALWAYS)) {
        ...
        int len = snprintf(
            ...
            app->fast_large_mono ? 1 : 0,
            app->restore_page_after_image ? 1 : 0,
            (int)app->led_encoding);
