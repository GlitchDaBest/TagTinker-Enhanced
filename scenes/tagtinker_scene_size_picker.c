...
static void setting_cb(void* ctx, uint32_t index) {
    ...
    const bool broadcast_text = app->broadcast_type == TagTinkerBroadcastText;
    TagTinkerTarget* target = NULL;
    if(!broadcast_text) {
    ...
    if(broadcast_text) {
        /* Experimental broadcast text has no target profile. Match the common
         * 2.2" Color M / HD M geometry used by the broadcast BMP path. */
        app->esl_width = 208U;
        app->esl_height = 112U;
    } else {
    ...
    if(!broadcast_text) {
        /* Auto-save targeted text only; broadcast text is a throwaway test. */
        tagtinker_recents_add(app, app->text_input_buf);
    }

    if(broadcast_text) {
        const uint8_t broadcast_plid[4] = {0};
        tagtinker_prepare_text_tx(app, broadcast_plid);
    } else {
...
void tagtinker_scene_size_picker_on_enter(void* ctx) {
    ...
    variable_item_list_add(
        app->var_item_list,
        (app->broadcast_type == TagTinkerBroadcastText) ? ">> Broadcast Text <<" :
                                                          ">> Send to Tag <<",
        0,
        NULL,
        app);
