...
static void nfc_scan_start_thread(TagTinkerApp* app) {
...
void tagtinker_scene_nfc_scan_on_enter(void* ctx) {
    TagTinkerApp* app = ctx;
    const bool rapid = scene_manager_get_scene_state(app->scene_manager, TagTinkerSceneNfcScan) ==
                       TagTinkerNfcScanRapid;

    popup_reset(app->popup);
    popup_set_header(
        app->popup, rapid ? "Quick NFC Scan" : "Scan NFC Tag", 64, 10, AlignCenter, AlignTop);
    if(rapid) {
        char line[48];
        snprintf(
            line,
            sizeof(line),
            "Hold tags to back\nAdded: %u\n[<-] Done",
            (unsigned)app->nfc_rapid_added);
        popup_set_text(app->popup, line, 64, 32, AlignCenter, AlignCenter);
    } else {
        popup_set_text(
...
bool tagtinker_scene_nfc_scan_on_event(void* ctx, SceneManagerEvent event) {
    ...
    if(event.event == NfcScanEventSuccess) {
        ...
        if(rapid) {
            if(app->nfc_rapid_added < 255U) app->nfc_rapid_added++;
            popup_reset(app->popup);
            popup_set_header(app->popup, "Quick NFC Scan", 64, 10, AlignCenter, AlignTop);
            char line[64];
            snprintf(
                line,
                sizeof(line),
                "Added %s\nTotal: %u\nScan next",
                app->targets[idx].name[0] ? app->targets[idx].name : app->barcode,
                (unsigned)app->nfc_rapid_added);
            popup_set_text(app->popup, line, 64, 32, AlignCenter, AlignCenter);
            view_dispatcher_switch_to_view(app->view_dispatcher, TagTinkerViewPopup);
            nfc_scan_start_thread(app);
