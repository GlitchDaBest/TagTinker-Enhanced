 ...
 * sets the default page in image options (wire page 0–7, same as broadcast).
 *
 * Default when no "_p<N>" suffix is page 0 so targeted BMPs hit the same slot
 * as broadcast "Change Page" at page 0 and the text path's first page (img_page
 * 1 → wire 0 in tagtinker_prepare_text_tx). */
static bool tagtinker_parse_dropped_filename(
    ...
    /* Stamp the target resolution for targeted sends. Broadcast image tests
     * have no target profile, so they require a filename resolution prefix. */
    if(target_w > 0U && target_h > 0U) {
        out->width = target_w;
        out->height = target_h;
    } else if(has_hint && w_hint > 0U && h_hint > 0U) {
        out->width = (uint16_t)w_hint;
        out->height = (uint16_t)h_hint;
    } else {
        return false;
    }
    ...
    if(has_hint) {
        out->resampled = (w_hint != out->width) || (h_hint != out->height);
    } else {
        out->resampled = true;
    }
...
static void dropped_images_load(TagTinkerApp* app) {
    const bool broadcast_image = app->broadcast_type == TagTinkerBroadcastImage;
    const TagTinkerTarget* target = NULL;
    uint16_t target_w = 0U;
    uint16_t target_h = 0U;

    if(!broadcast_image) {
        if(app->selected_target < 0 || app->selected_target >= app->target_count) return;
        target = &app->targets[app->selected_target];
        if(!tagtinker_target_supports_graphics(target)) return;
        target_w = target->profile.width;
        target_h = target->profile.height;
    }
...
void tagtinker_scene_synced_image_list_on_enter(void* ctx) {
    ...
    submenu_set_header(
        app->submenu,
        (app->broadcast_type == TagTinkerBroadcastImage) ? "Broadcast BMP EXP" : "Set Image");
    ...
    if(app->synced_image_count == 0) {
        ...
        submenu_add_item(
            app->submenu,
            (app->broadcast_type == TagTinkerBroadcastImage) ? "Need 208x112_name.bmp" :
                                                               "Use Image Prep page",
            0,
...
bool tagtinker_scene_synced_image_list_on_event(void* ctx, SceneManagerEvent event) {
    ...
    if(broadcast_image) {
        ...
        tagtinker_prepare_bmp_tx(
            app, broadcast_plid, image->image_path, image->width, image->height, image->page);
    } else {
