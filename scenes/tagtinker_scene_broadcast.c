...
static const char* repeat_labels[] = {"50", "100", "200", "400", "800", "1200"};
static const uint16_t repeat_values[] = {50, 100, 200, 400, 800, 1200};
...
void tagtinker_scene_broadcast_on_enter(void* context) {
    ...
    if(app->broadcast_type == TagTinkerBroadcastFlipPage ||
       ...
       app->broadcast_type == TagTinkerBroadcastRave ||
    ...
    if(app->broadcast_type == TagTinkerBroadcastFlipPage) {
        s_transmit_row_index = 5;
    } else if(app->broadcast_type == TagTinkerBroadcastPageParty ||
              app->broadcast_type == TagTinkerBroadcastRave ||
              app->broadcast_type == TagTinkerBroadcastChaos ||
              app->broadcast_type == TagTinkerBroadcastLed ||
              app->broadcast_type == TagTinkerBroadcastLedSweep) {
        /* Duration + Forever + Repeats + Repeat, then Transmit */
        s_transmit_row_index = 4;
    } else {
        s_transmit_row_index = 2;
    }
     ...
     * the broadcast.
     *
     * The focused row is intentionally row 0 (first setting), not
     * Transmit. Reason: the OK key-press that picked us out of
     * the previous Submenu generates a key-release input event that
     * arrives just after this scene's VIL becomes active. If the
     * focus were on the Transmit row at that moment the stale OK
...
void tagtinker_scene_broadcast_on_exit(void* context) {
    ...
    scene_manager_set_scene_state(
        app->scene_manager, TagTinkerSceneBroadcast, TagTinkerBroadcastSceneNormal);
