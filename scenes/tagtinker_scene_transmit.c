...
/* IR image-param frame carries byte_count as uint16_t; payloads above this
 * must be sent in vertical stripes (800x480x2 raw = 96000 B otherwise wraps). */
#define TX_MAX_IMAGE_PARAM_BYTES 60000U
...
/* SmartTAG Color graphic tags (2.2" 1503/1605/1695 and 7.3" 1358): same MCU path as
 * SmartTags, but (a) comp_type raw is required on many builds, and (b) IR timing is
 * pickier — without enough inter-frame time and data repeats the tag may receive RF
 * activity yet never ACK. SmartTag HD tolerates the default short gaps. */
static bool tx_is_hd110_panel(const TagTinkerApp* app) {
    if(!app || app->selected_target < 0 || app->selected_target >= app->target_count) {
        return false;
    }
    uint16_t tc = app->targets[app->selected_target].profile.type_code;
    return (tc == TAGTINKER_TYPE_HD110) || (tc == TAGTINKER_TYPE_HD110_RED);
}

static bool tx_is_pricer_color_m_tag(const TagTinkerApp* app) {
    if(app && app->image_tx_job.broadcast) {
        return false;
    }
    if(!app || app->selected_target < 0 || app->selected_target >= app->target_count) {
        return false;
    ...
    uint16_t tc = app->targets[app->selected_target].profile.type_code;
    return (tc == 1503U) || (tc == 1605U) || (tc == 1695U) || (tc == 1358U);
}

/* SmartTAG Color (HD 200) 7.3" — very long striped uploads; extra IR headroom. */
static bool tx_is_large_color_panel(const TagTinkerApp* app) {
    if(!tx_is_pricer_color_m_tag(app)) return false;
...
static bool tx_send_refresh(TagTinkerApp* app, const uint8_t plid[4]) {
    ...
    uint16_t reps = tx_is_broadcast_text(app) ? 65U :
                    ...
                    20U;
    return tx_send_frame(app, frame, len, reps);
}
...
static bool tx_send_payload_frames(
    ...
    uint16_t data_repeats = app->data_frame_repeats;
    if(tx_is_broadcast_text(app)) {
        ...
        if(data_repeats < 6U) data_repeats = 6U;
        if(data_repeats > 10U) data_repeats = 10U;
    }
    for(size_t i = 0; ok && i < frame_count; i++) {
        ...
        if(tx_is_broadcast_text(app) || tx_is_broadcast_bmp(app)) {
            ...
            if(ok) furi_delay_ms(2U);
        } else if(ok && ((i + 1U) % 32U) == 0U && (i + 1U) < frame_count) {
            furi_delay_ms(1);
        }
    }
...
static bool tx_send_image_chunk(
    ...
    uint32_t frame_index_base,
    bool send_image_start,
    size_t* out_data_frames) {
    TagTinkerImagePayload payload;
    size_t pixel_count = (size_t)stripe_width * stripe_pixel_h;
    if(!tagtinker_encode_planes_payload(
           primary_pixels,
           secondary_pixels,
           pixel_count,
           tx_effective_compression(app),
           &payload)) {
        return false;
    }

    bool ok = tx_send_payload_frames(
        app,
        plid,
        &payload,
        page,
    ...
    if(out_data_frames) {
        *out_data_frames = payload.byte_count / TAGTINKER_IMAGE_DATA_BYTES_PER_FRAME;
    }
...
static uint32_t tx_chunk_settle_delay_ms(uint16_t width, uint16_t height, bool color_clear) {
    ...
    if(delay_ms > 2500U) delay_ms = 2500U;
    return delay_ms;
}
...
static uint16_t tx_pick_chunk_height(uint16_t width, uint16_t height, bool second_plane) {
    ...
    /* Wide panels (800 px): use 16 KB/plane → ~20 rows/stripe and fewer seams.
     * Narrow tags keep 8 KB to leave heap headroom after BLE teardown. */
    size_t per_plane_budget = (width >= 400U) ? 16384U : 8192U;
    uint16_t chunk_h = (uint16_t)(per_plane_budget / width);
    if(chunk_h == 0U) chunk_h = 1U;
    if(chunk_h > height) chunk_h = height;

    /* Align stripe height to 16 rows on large panels (e.g. 800x480 → 30 stripes). */
    if(width >= 400U && height >= 64U && chunk_h >= 16U) {
        chunk_h = (uint16_t)(chunk_h & ~15U);
        if(chunk_h == 0U) chunk_h = 16U;
    } else if(chunk_h >= 8U) {
...
static bool tx_send_full_payload(
    ...
    if(ok) {
        ok = tx_send_payload_frames(
            app, plid, payload, page, width, height, pos_x, pos_y, 0U, true);
    }
    if(ok) {
        furi_delay_ms(
...
static bool tx_send_full_text_image(TagTinkerApp* app) {
    ...
    bool ok = tagtinker_encode_planes_payload(
        primary, secondary, pixel_count, tx_effective_compression(app), &payload);
    free(primary);
    free(secondary);
...
static bool tx_bmp_fill_stripe_pixels(
    File* file,
    ...
    bool has_secondary_in_bmp,
    uint16_t plane2_off_rows,
    uint8_t* primary,
    uint8_t* secondary) {
    uint8_t row_buf[128];
    uint16_t cached_src_y = UINT16_MAX;
    bool ok = true;
    size_t stride = (size_t)tx_width;

    for(uint16_t row = 0; ok && row < stripe_h; row++) {
        uint16_t out_y = (uint16_t)(y0 + row);
        if(out_y >= tx_height) break;

        uint16_t src_y = bmp_map_y(out_y, tx_height, info->height);
        if(src_y != cached_src_y) {
            if(!bmp_read_row_at(file, info, src_y, 0U, row_buf)) {
                ok = false;
                break;
            }
            cached_src_y = src_y;
        ...
        for(uint16_t x = 0; x < tx_width; x++) {
            primary[(size_t)row * stride + x] =
                bmp_read_pixel(row_buf, bmp_map_x(x, tx_width, info->width));
        }
    }
    ...
    if(ok && use_second_plane) {
        ...
        if(has_secondary_in_bmp) {
            for(uint16_t row = 0; ok && row < stripe_h; row++) {
                ...
                uint16_t src_y = bmp_map_y(out_y, tx_height, info->height);
                if(src_y != cached_src_y) {
                    if(!bmp_read_row_at(file, info, src_y, plane2_off_rows, row_buf)) {
                        ok = false;
                        break;
                    }
                    cached_src_y = src_y;
                ...
                for(uint16_t x = 0; x < tx_width; x++) {
                    secondary[(size_t)row * stride + x] =
                        bmp_read_pixel(row_buf, bmp_map_x(x, tx_width, info->width));
                }
            }
        } else {
            memset(secondary, 1, stride * stripe_h);
        }
    }
...
static bool tx_stream_bmp_chunked(
    TagTinkerApp* app,
    File* file,
    const TxBmpInfo* info,
    uint16_t tx_width,
    ...
    uint16_t plane2_off_rows) {
    const TagTinkerImageTxJob* job = &app->image_tx_job;
    uint16_t chunk_h = tx_pick_chunk_height(tx_width, tx_height, use_second_plane);
    size_t stripe_pixels = (size_t)tx_width * chunk_h;
    uint8_t* primary = malloc(stripe_pixels);
    uint8_t* secondary = use_second_plane ? malloc(stripe_pixels) : NULL;
    ...
    FURI_LOG_I(
        TAGTINKER_TAG,
        "BMP TX chunked w=%u h=%u stripe_h=%u",
        (unsigned)tx_width,
        (unsigned)tx_height,
        (unsigned)chunk_h);
    ...
    for(uint16_t y = 0; ok && y < tx_height; y = (uint16_t)(y + chunk_h)) {
        uint16_t actual_h = (uint16_t)(tx_height - y);
        if(actual_h > chunk_h) actual_h = chunk_h;

        ok = tx_bmp_fill_stripe_pixels(
            file,
            info,
            tx_width,
            tx_height,
...
static bool tx_send_ping_led_raw(
    TagTinkerApp* app,
    const uint8_t plid[4],
    const uint8_t blink_payload[6],
    uint16_t ping_rep,
    uint16_t led_rep) {
    UNUSED(app);
    uint8_t ping_frame[TAGTINKER_MAX_FRAME_SIZE];
    uint8_t led_frame[TAGTINKER_MAX_FRAME_SIZE];

    size_t ping_len = tagtinker_make_ping_frame(ping_frame, plid);
    bool ok = tagtinker_ir_transmit(ping_frame, ping_len, ping_rep, 10);
    if(ok) furi_delay_ms(80);
    if(ok) {
        size_t led_len =
            tagtinker_make_addressed_frame(led_frame, plid, blink_payload, 6U);
        ok = tagtinker_ir_transmit(led_frame, led_len, led_rep, 10);
    }
    return ok;
}

static TagTinkerLedEncoding tx_led_encoding(const TagTinkerApp* app) {
    if(!app) return TagTinkerLedEncSubcmd;
    if(app->led_encoding > (uint8_t)TagTinkerLedEncPricerMask) return TagTinkerLedEncSubcmd;
...
static bool tx_send_ping_led_ex(
    ...
    if(enc == TagTinkerLedEncBoth) {
        tagtinker_fill_led_blink_subcmd_map(
            blink_payload, app->duration, app->forever, color_index, true);
        bool ok = tx_send_ping_led_raw(app, plid, blink_payload, ping_rep, led_rep);
        if(ok) furi_delay_ms(80);
        if(ok) {
            tagtinker_fill_led_blink_subcmd_map(
                blink_payload, app->duration, app->forever, color_index, false);
            ok = tx_send_ping_led_raw(app, plid, blink_payload, ping_rep, led_rep);
        }
        return ok;
    }
...
static bool tx_send_ping_page(
    ...
    uint16_t page_rep) {
    uint8_t ping_frame[TAGTINKER_MAX_FRAME_SIZE];
    uint8_t page_frame[TAGTINKER_MAX_FRAME_SIZE];
    uint8_t page_payload[6];

    tagtinker_fill_page_select_payload(page_payload, page, app->forever, app->duration);
    ...
    if(ok) {
        size_t page_len =
            tagtinker_make_addressed_frame(page_frame, plid, page_payload, sizeof(page_payload));
        ok = tagtinker_ir_transmit(page_frame, page_len, page_rep, 10);
    }
    return ok;
}

static uint32_t tx_party_delay_ms(const TagTinkerApp* app) {
    uint32_t ms = (uint32_t)app->duration * 1000U;
    if(ms < 400U) ms = 400U;
    if(ms > 8000U) ms = 8000U;
    return ms;
}

static bool tx_led_sweep(TagTinkerApp* app) {
    if(app->target_count == 0U) return false;

    uint16_t ping_rep = app->repeats;
    uint16_t led_rep = app->repeats;
    if(ping_rep < 120U) ping_rep = 120U;
    if(led_rep < 80U) led_rep = 80U;

    bool ok = true;
    for(uint8_t i = 0; ok && i < app->target_count; i++) {
        if(!app->tx_active) return false;
        ok = tx_send_ping_led(app, app->targets[i].plid, app->led_color, ping_rep, led_rep);
        if(ok && (i + 1U) < app->target_count) furi_delay_ms(200);
    }
    return ok;
}

static bool tx_led_color_sweep(TagTinkerApp* app) {
    if(app->target_count == 0U) return false;
    ...
    for(uint8_t i = 0; ok && i < app->target_count; i++) {
        if(!app->tx_active) return false;
        for(uint8_t c = 0; ok && c < TAGTINKER_LED_COLOR_COUNT; c++) {
            ok = tx_send_ping_led(app, app->targets[i].plid, c, ping_rep, led_rep);
            if(ok) furi_delay_ms(120);
        }
        if(ok && (i + 1U) < app->target_count) furi_delay_ms(180);
    }
    return ok;
}

static bool tx_led_subcmd_scan(TagTinkerApp* app) {
    const uint8_t* plid = NULL;
    const uint8_t broadcast_plid[4] = {0};
    uint16_t ping_rep = 160U;
    uint16_t led_rep = 120U;

    if(app->selected_target >= 0 && app->selected_target < (int8_t)app->target_count) {
        plid = app->targets[app->selected_target].plid;
        ping_rep = 140U;
        led_rep = 100U;
    } else {
        plid = broadcast_plid;
    ...
    app->led_scan_phase = 0U;
    app->led_scan_cmd = 0U;
    for(uint8_t cmd = 0xC0U; cmd <= 0xCFU && app->tx_active; cmd++) {
        app->led_scan_cmd = cmd;
        uint8_t payload[6];
        payload[0] = 0x06;
        payload[1] = cmd;
        payload[2] = 0x00;
        payload[3] = 0x00;
        payload[4] = 0x00;
        payload[5] = (uint8_t)TAGTINKER_LED_FLASH_DURATION_SEC;
        if(!tx_send_ping_led_raw(app, plid, payload, ping_rep, led_rep)) return false;
        furi_delay_ms(500);
    }

    app->led_scan_phase = 1U;
    for(uint8_t idx = 0U; idx < TAGTINKER_LED_COLOR_COUNT && app->tx_active; idx++) {
        app->led_scan_cmd = idx;
        uint8_t payload[6];
        payload[0] = 0x06;
        payload[1] = 0x49;
        payload[2] = idx;
    ...
    app->led_scan_phase = 3U;
    for(uint8_t idx = 0U; idx < TAGTINKER_LED_COLOR_COUNT && app->tx_active; idx++) {
        app->led_scan_cmd = idx;
        uint8_t payload[6];
        payload[0] = 0x06;
        payload[1] = 0x49;
        payload[2] = (uint8_t)(1U << (idx & 7U));
        payload[3] = 0x00;
        payload[4] = 0x00;
        payload[5] = (uint8_t)TAGTINKER_LED_FLASH_DURATION_SEC;
        if(!tx_send_ping_led_raw(app, plid, payload, ping_rep, led_rep)) return false;
...
static bool tx_led_party_once(TagTinkerApp* app) {
    const uint8_t plid[4] = {0};
    uint16_t ping_rep = app->repeats;
    uint16_t led_rep = app->repeats;
    if(ping_rep < 200U) ping_rep = 200U;
    if(led_rep < 120U) led_rep = 120U;

    /* Classic patterns that flash on F62/HD S; then full scan map for Color M. */
    static const uint8_t classic_party[] = {2U, 4U, 3U, 1U, 0U, 5U, 6U};

    bool ok = true;
    for(size_t i = 0; ok && i < COUNT_OF(classic_party) && app->tx_active; i++) {
        uint8_t payload[6];
        tagtinker_fill_led_blink_subcmd_map(
            payload, app->duration, app->forever, classic_party[i], true);
        ok = tx_send_ping_led_raw(app, plid, payload, ping_rep, led_rep);
        if(ok) furi_delay_ms(250);
    }
    for(uint8_t c = 0; ok && c < TAGTINKER_LED_COLOR_COUNT && app->tx_active; c++) {
        uint8_t payload[6];
        tagtinker_fill_led_blink_subcmd_map(payload, app->duration, app->forever, c, false);
        ok = tx_send_ping_led_raw(app, plid, payload, ping_rep, led_rep);
        if(ok) furi_delay_ms(200);
    }
    return ok;
}

static bool tx_led_party_broadcast(TagTinkerApp* app) {
    bool ok = true;
    do {
...
static bool tx_page_party_once(TagTinkerApp* app) {
    uint8_t frame[TAGTINKER_MAX_FRAME_SIZE];
    uint16_t rep = app->repeats;
    if(rep < 100U) rep = 100U;
    const uint32_t pause = tx_party_delay_ms(app);

    bool ok = true;
    for(uint8_t pg = 0; ok && pg < 8U && app->tx_active; pg++) {
        size_t len = tagtinker_build_broadcast_page_frame(
            frame, pg, app->forever, app->duration);
        ok = tagtinker_ir_transmit(frame, len, rep, 10);
        if(ok) furi_delay_ms(pause);
    }
    return ok;
}

static bool tx_page_party(TagTinkerApp* app) {
    bool ok = true;
    do {
        ok = tx_page_party_once(app);
    } while(ok && app->tx_spam && app->tx_active);
    return ok;
}
...
static bool tx_chaos(TagTinkerApp* app) {
    bool ok = true;
    do {
        if(!app->tx_active) return false;
...
static bool tx_led_locator(TagTinkerApp* app) {
    if(app->selected_target < 0 || app->selected_target >= (int8_t)app->target_count) {
        return false;
    }
    const uint8_t* plid = app->targets[app->selected_target].plid;
    uint16_t ping_rep = 120U;
    uint16_t led_rep = 80U;

    while(app->tx_active) {
        if(!tx_send_ping_led(app, plid, app->led_color, ping_rep, led_rep)) return false;
        furi_delay_ms(450);
    }
    return true;
}

static bool tx_led_rainbow_target(TagTinkerApp* app) {
    if(app->selected_target < 0 || app->selected_target >= (int8_t)app->target_count) {
        return false;
    }
    const uint8_t* plid = app->targets[app->selected_target].plid;
    ...
    for(uint8_t c = 0; ok && c < TAGTINKER_LED_COLOR_COUNT; c++) {
        if(!app->tx_active) return false;
        ok = tx_send_ping_led(app, plid, c, ping_rep, led_rep);
        if(ok) furi_delay_ms(200);
    }
    return ok;
}

static bool tx_restore_broadcast_page0(TagTinkerApp* app) {
    UNUSED(app);
    uint8_t frame[TAGTINKER_MAX_FRAME_SIZE];
    size_t len = tagtinker_build_broadcast_page_frame(frame, 0U, false, 5U);
    return tagtinker_ir_transmit(frame, len, 120U, 10);
}
...
static bool tx_stream_bmp_image(TagTinkerApp* app) {
    ...
    if(app->selected_target >= 0 && app->selected_target < app->target_count) {
        ...
        FURI_LOG_I(
            TAGTINKER_TAG,
            "BMP TX type=%04u pricerColorM=%c broadcast=%c",
            (unsigned)tc,
            tx_is_pricer_color_m_tag(app) ? 'Y' : 'N',
            job->broadcast ? 'Y' : 'N');
    } else if(job->broadcast) {
        FURI_LOG_I(TAGTINKER_TAG, "BMP TX broadcast=Y");
    }
    ...
    bool use_second_plane = app->color_clear || accent_capable ||
                            (job->broadcast && has_secondary_in_bmp);
...
static int32_t tx_thread_callback(void* context) {
    ...
    do {
        if(app->broadcast_type == TagTinkerTransmitLedLocator) {
        ...
        } else if(app->broadcast_type == TagTinkerBroadcastLedColorSweep) {
            ok = tx_led_color_sweep(app);
        } else if(app->broadcast_type == TagTinkerBroadcastLedColors) {
            ok = tx_led_party_broadcast(app);
        } else if(app->broadcast_type == TagTinkerBroadcastLedScan) {
            ok = tx_led_subcmd_scan(app);
        } else if(app->broadcast_type == TagTinkerBroadcastPageParty) {
            ok = tx_page_party(app);
        } else if(app->broadcast_type == TagTinkerBroadcastRave) {
            ok = tx_rave(app);
        ...
        if(app->tx_spam && app->tx_active && !app->image_tx_job.mode &&
           app->broadcast_type != TagTinkerBroadcastLedSweep &&
           app->broadcast_type != TagTinkerBroadcastLedColorSweep &&
           app->broadcast_type != TagTinkerBroadcastLedScan &&
           app->broadcast_type != TagTinkerBroadcastPageParty &&
           app->broadcast_type != TagTinkerBroadcastChaos &&
...
static void transmit_draw_cb(Canvas* canvas, void* _model) {
    ...
    if(app->tx_active) {
        if(app->broadcast_type == TagTinkerTransmitLedLocator) {
            ...
            canvas_draw_str_aligned(canvas, 64, 28, AlignCenter, AlignTop, "Color sweep tags");
        } else if(app->broadcast_type == TagTinkerBroadcastLed) {
            canvas_draw_str_aligned(canvas, 64, 28, AlignCenter, AlignTop, "LED party (all tags)");
        } else if(app->broadcast_type == TagTinkerBroadcastLedColors) {
            canvas_draw_str_aligned(canvas, 64, 28, AlignCenter, AlignTop, "LED party");
        } else if(app->broadcast_type == TagTinkerBroadcastLedScan) {
            char line[32];
            if(app->led_scan_phase == 0U) {
                snprintf(line, sizeof(line), "Scan sub 0x%02X", (unsigned)app->led_scan_cmd);
            } else if(app->led_scan_phase == 1U) {
                snprintf(line, sizeof(line), "Scan 49 b2 #%u", (unsigned)app->led_scan_cmd);
            } else if(app->led_scan_phase == 2U) {
                snprintf(line, sizeof(line), "Scan 49 b3 #%u", (unsigned)app->led_scan_cmd);
            } else {
                snprintf(line, sizeof(line), "Scan 49 mk #%u", (unsigned)app->led_scan_cmd);
            }
            canvas_draw_str_aligned(canvas, 64, 28, AlignCenter, AlignTop, line);
        } else if(app->broadcast_type == TagTinkerBroadcastPageParty) {
            canvas_draw_str_aligned(canvas, 64, 28, AlignCenter, AlignTop, "Page party 0-7");
        } else if(app->broadcast_type == TagTinkerBroadcastRave) {
            canvas_draw_str_aligned(canvas, 64, 28, AlignCenter, AlignTop, "Rave mode");
        } else if(app->broadcast_type == TagTinkerBroadcastChaos) {
            canvas_draw_str_aligned(canvas, 64, 28, AlignCenter, AlignTop, "Chaos timer");
        } else {
...
void tagtinker_scene_transmit_on_enter(void* context) {
    ...
    app->tx_active = true;
    if(furi_thread_get_state(app->tx_thread) != FuriThreadStateStopped) {
