...
void tagtinker_scene_target_menu_on_enter(void* ctx) {
    ...
    submenu_add_item(app->submenu, "+ Quick Scan NFC", TargetMenuQuickScanNfc, target_menu_cb, app);
    submenu_add_item(app->submenu, "+ Type Barcode", TargetMenuAddNew, target_menu_cb, app);
    submenu_add_item(app->submenu, "? Help / Guide", TargetMenuHelp, target_menu_cb, app);
...
bool tagtinker_scene_target_menu_on_event(void* ctx, SceneManagerEvent event) {
    ...
    if(event.event == TargetMenuHelp) {
        tagtinker_show_text_box_help(app, TagTinkerTextBoxHelpTarget);
        return true;
    }

    if(event.event == TargetMenuScanNfc) {
        scene_manager_set_scene_state(
            app->scene_manager, TagTinkerSceneNfcScan, TagTinkerNfcScanNormal);
        scene_manager_next_scene(app->scene_manager, TagTinkerSceneNfcScan);
        return true;
    }

    if(event.event == TargetMenuQuickScanNfc) {
        app->nfc_rapid_added = 0U;
        scene_manager_set_scene_state(
            app->scene_manager, TagTinkerSceneNfcScan, TagTinkerNfcScanRapid);
        scene_manager_next_scene(app->scene_manager, TagTinkerSceneNfcScan);
        return true;
    }
