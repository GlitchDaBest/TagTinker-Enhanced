...
bool tagtinker_scene_barcode_input_on_event(void* ctx, SceneManagerEvent event) {
    ...
    if(!tagtinker_barcode_to_profile(app->barcode, &profile)) {
        ...
        tagtinker_barcode_to_type(app->barcode, &type_code);
        static char unsupported_body[96];
        snprintf(
            unsupported_body,
            sizeof(unsupported_body),
            "No profile for type %04u\n(barcode pos 13-16)",
            (unsigned)type_code);

        widget_reset(app->widget);
        widget_add_string_element(app->widget, 64, 10, AlignCenter, AlignTop, FontPrimary, "Unsupported Tag");
        widget_add_string_multiline_element(
            app->widget, 64, 28, AlignCenter, AlignTop, FontSecondary, unsupported_body);
